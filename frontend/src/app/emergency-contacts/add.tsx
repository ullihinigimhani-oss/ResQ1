...
export default function AddEmergencyContactScreen() {
  ...
  const handleSubmit = async () => {
    ...
    if (!token) {
      setErrorMessage('You must be logged in to add emergency contacts.');
      return;
    }
    ...
    try {
      await createEmergencyContact(token, {
        name: name.trim(),
      ...
      if (isEmergencyContactApiError(err)) {
        ...
        setErrorMessage('Failed to add emergency contact. Please try again.');
      }
    } finally {
      setLoading(false);
    }
      ...
      <KeyboardAvoidingView
        ...
        <ScrollView showsVerticalScrollIndicator={false}>
          <AppHeader
            ...
            subtitle="Register a trusted person who can be reached quickly during emergency evacuations."
            onBack={() => router.back()}
          />
